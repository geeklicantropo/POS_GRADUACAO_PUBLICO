...
# ---------------------------------------------------------------------
def train_model(classification_type, num_epochs=30):
    """
    ...
    """
    # Ensure that classification_type is provided and valid
    ...
    # Create data loaders with appropriate batch size and num_workers
    ...
    for epoch in range(num_epochs):
        ...
        total_training_time += epoch_duration  # Accumulate total training time
    ...
    total_time = total_training_time + classification_time
    ...
    metrics_summary = pd.DataFrame({
        ...
        'Value': [
            ...
            val_accuracies[-1],
            roc_auc,
            training_time := total_training_time,  # Use accumulated training time
            classification_time,
            total_time
        ]
    })
    ...
    if classification_type == 'binary':
        ...
        # Plot ROC curve
        plt.figure()
        plt.plot(fpr, tpr, label=f'ROC Curve (AUC = {roc_auc_value:.4f})', color='darkorange', lw=2)
        ...
        # For multi-class classification, compute ROC curve for each class
        plt.figure()
        fpr = {}
        tpr = {}
        roc_auc = {}
        for i in range(num_classes):
            fpr[i], tpr[i], _ = roc_curve(all_labels_np == i, probs[:, i])
            roc_auc[i] = auc(fpr[i], tpr[i])
            plt.plot(fpr[i], tpr[i], lw=2, label=f'Class {idx_to_class[i]} (AUC = {roc_auc[i]:0.4f})')
        plt.plot([0, 1], [0, 1], linestyle='--', color='gray', lw=2)
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
    ...
    logging.info(f"Total execution time for train_static.py: {total_execution_time/60:.2f} minutes.")
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------
    # Start Training for Static Data (Binary and Multiclass Classification)
    # -----------------------------------------------------------------
